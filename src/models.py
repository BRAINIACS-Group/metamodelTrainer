...
def improve(model,label_fn,ranges,k=10,pool_size=None):
    ...
    new_model.comment("This model was generated by an improvement function.")
...
def improve_random(model,label_fn,ranges,k=8,pool_size=None):
    ...
    for _ in range(k):
        ...
        while distance_to_sample(X[0],P_T,ranges) < 0.75*min_distance(P_T,ranges): X = RandSample(ranges,1)
        P_T = P_T.append(X)
