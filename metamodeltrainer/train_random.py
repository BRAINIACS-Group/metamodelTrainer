...
save_path = Path(cwd,f"models_HBE_05_16_red_stress_compten_{random_str}_{datetime.today().strftime('%Y%m%d')}")
...
for i in range(i_start,95):
    ...
    improve_fn = improve_random if RANDOM else improve
    model_bis = improve_fn(model,label_fn,PSpace,k=5)
