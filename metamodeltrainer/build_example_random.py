...
from metamodeltrainer.models import HyperParameters, RecModel, improve, load_model, improve_random           #Neural Network management
from metamodeltrainer.run_simulation import label


cwd = Path(__file__).resolve().parents[1]
save_path = Path(cwd,f"models_HBE_05_16_red_random{datetime.today().strftime('%Y%m%d')}")
...
for i in range(95):
    model_bis = improve_random(model,label_fn,PSpace,k=5)
